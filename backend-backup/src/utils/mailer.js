...
export async function sendOrderStatusUpdate(order) {
  ...
  const statusLabel =
    statusLabels[order.status] || "Order Updated";
        ...
        <div style="max-width:620px;margin:auto;background:#ffffff;border-radius:14px;overflow:hidden;">
          ...
          <div style="padding:30px;">
            ...
            <div style="background:#f7f7f7;padding:18px;border-radius:10px;margin-top:20px;">
              ...
              ${
                courierName
                  ? `<p><strong>Courier:</strong> ${escapeHtml(courierName)}</p>`
                  : ""
              }

              ${
                awbNumber
                  ? `<p><strong>AWB Number:</strong> ${escapeHtml(awbNumber)}</p>`
                  : ""
              }
...
Thank you for shopping with GenZeReal.
  ...
  console.log(
    `Order status email sent to ${order.customerEmail}: ${info.messageId}`
  );
