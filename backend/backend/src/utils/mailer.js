...
export async function sendOrderStatusUpdate(order) {
  ...
  const statusLabel = labels[order.status] || "Order Updated";
  const statusMessage = messages[order.status] || "Your order status has been updated.";
  const courierName = order.courier?.name || "";
  const awbNumber = order.courier?.awbNumber || "";
  const trackingUrl = order.courier?.trackingUrl || "";
  const cancellationReason = order.cancellation?.reason || "";
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;background:#f3f3f3;padding:24px;">
      <div style="max-width:620px;margin:auto;background:#ffffff;border-radius:14px;overflow:hidden;">
        <div style="background:#0a0a0a;color:#ffffff;padding:28px;text-align:center;">
          <h1 style="margin:0;letter-spacing:3px;">GENZEREAL</h1>
          <p style="margin:8px 0 0;color:#cccccc;">WEAR WHAT'S REAL</p>
        </div>
        <div style="padding:30px;">
          <p>Hi ${escapeHtml(order.customerName || "Customer")},</p>
          <h2>${escapeHtml(statusLabel)}</h2>
          <p>${escapeHtml(statusMessage)}</p>
          <div style="background:#f7f7f7;padding:18px;border-radius:10px;margin-top:20px;">
            ...
            ${awbNumber ? `<p><strong>AWB Number:</strong> ${escapeHtml(awbNumber)}</p>` : ""}
            ${trackingUrl ? `<p><a href="${escapeHtml(trackingUrl)}">Track your order</a></p>` : ""}
            ${cancellationReason ? `<p><strong>Cancellation Reason:</strong> ${escapeHtml(cancellationReason)}</p>` : ""}
          </div>
        </div>
      </div>
    </div>
...
AWB Number: ${awbNumber || "Not available"}
Tracking URL: ${trackingUrl || "Not available"}
Cancellation Reason: ${cancellationReason || "Not applicable"}
